...
date: 2025-11-25
description: "Building an AI agent to accelerate clinical research study development to PDF specifications into study configurations using natural language, through three iterations of learning and refinement."

---

This year at Sano, I transitioned from being a majority-time manager CTO here to being a majority-time technology focusssed CTO driving to adoption of AI across our product and across the business more generally. The new AI tooling had given me so much motivation and drive that it felt like a natural transition to spend more time as an individual contributor and get back into building.

A big goal for this year within the company was to help accelerate the development of clinical research studies on the Sano Portal for both delivery and commercial teams. It takes time and effort to translate study specification details that live within PDF documents into the equivalent representation using the Admin Product tools we had spent so many years crafting.
...
The high-level system is described by the following diagram. The Sano Agent acts as an intermediate entity which performs API requests via a sequence of specific tools to carry out a user's intent. Crucially, the authorisation headers from the user are included such that the agent is only able to perform actions that the user is able to perform themselves.
...
1. The client sends the user prompt alongside the user cookie and JSON Web Token (JWT), which are used later for authenticating web requests alongside the previous message history.
...
The Agent Server is a standalone service implemented with FastAPI within our mono-repository, similar to how are existing services are structured.

A key component of the agent server prompt is a reference to all the BlockType classes that we currently support. This enables the Study Agent to accurately select the correct Block Type and Block Data based on a user prompt, but it also introduces a dependency on the core server codebase. After a debate with the team we decided that it was sufficient at this stage to use the code that defines all these Blocktype entities as context for the Agent. In the future, this context could be automatically generated via an OpenAPI spec or by even hosting our own MCP server directly from our OpenAPI specification. Given the uncertainties in this approach, we decided not to pursue this avenue straight away and instead launch, gather feedback, and generate momentum in this product direction before investing in this longer-term technical approach.


The backend exposes REST endpoints, such as `/agent/execute-prompt/stream`, which calls an Agent implemented with [PydanticAI](https://ai.pydantic.dev/), and streams back the response after each Tool Call completes until termination.
...
The component collects the context of the Study that the user is currently looking at and collects together all the Flows, Blocks, Stages, Stage Rules, Triggers, that particular study. It adds this context to the prompt that is sent to the Agent service. This allows the user to reference study components (e.g., Blocks and Stages) with loose descriptions, and the Agent can determine which components the user is referring to.
...
Here is an example of a test case designed to create a study given a prompt and then assert that correct entities have indeed been created.
...
Sano Agent V1 focused on implementing the core pieces of functionality outlined in the vision and to generate buy-in across the company. It served as a successful proof of concept to help people see the vision about what could be achieved if we invested more in this technology approach. To simplify the complexity of the initial implementation, we specifically did not include PDF upload as a requirement at this stage.
<figure>
...
Sano Agent V2 added PDF upload support and required me to break out the previously monolithic agent into a multi-agent system. My initial approach was to implement a retrieval-augmented generation (RAG) system to split large PDFs into chunks, generate embeddings from them, and augment generations with relevant chunks. However, when testing early prototypes, I found that this introduced significant issues with the order of questions within pre-screening Flows, which is extremely important. I resorted to using long-context models such as Gemini 2.5 and later Claude Sonnet 4.5 and ChatGPT 4.1, whose 1-million-context-length was sufficient to ingest full-length PDFs, summarise them, extract the relevant content, and pass it to the main model to create the necessary study components.
